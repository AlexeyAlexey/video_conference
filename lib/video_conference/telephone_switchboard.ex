defmodule VideoConference.TelephoneSwitchboard do
  alias VideoConference.TelephoneSwitchboard.HostPublicKey
  alias VideoConference.TelephoneSwitchboard.AuthToken

  def get_public_key_by_host(host) when is_binary(host) do
    HostPublicKey.fetch(host)
  end

  def get_connection_options_for("video", "local", "phone_call" = type, token_params)
      when is_map(token_params) do
    token_params =
      token_params
      |> Map.put("type", type)

    %{
      "switchboard_video_uri" =>
        "https://#{System.fetch_env!("HTTP3_SERVER_HOST")}:#{System.fetch_env!("HTTP3_SERVER_PORT")}/video?auth_token=#{switchboard_auth_token(token_params)}",
      "switchboard_video_server_cert_hash" => System.fetch_env!("HTTP3_SERVER_CERT_HASH")
    }
  end

  def get_connection_options_for("audio", "local", "phone_call" = type, token_params)
      when is_map(token_params) do
    token_params =
      token_params
      |> Map.put("type", type)

    %{
      "switchboard_audio_uri" =>
        "https://#{System.fetch_env!("HTTP3_SERVER_HOST")}:#{System.fetch_env!("HTTP3_SERVER_PORT")}/audio?auth_token=#{switchboard_auth_token(token_params)}",
      "switchboard_audio_server_cert_hash" => System.fetch_env!("HTTP3_SERVER_CERT_HASH")
    }
  end

  defp switchboard_auth_token(params) when is_map(params) do
    {:ok, auth_token} = AuthToken.generate_token(params)
    auth_token
  end
end
